...
#define CommandHandler(name) \
	SHELLERR name(uint32_t Command, std::string_view * Args, uint32_t NumArgs, boost::static_string<RESOURCE_SHELL_OUTPUT_SIZE> &Output, uint32_t &Error, bool (*InterruptOccurred)(void))
...
// TODO all the Get##class_name functions created by STATIC_REFS and STATIC_VALS can be replaced with a single function that takes the size of the type as a parameter
...
/// See README.md for documentation on the IOResource class
class IOResource {
